#!/usr/bin/env node
/**
 * VAL-0 — Piloto admisiones en modo seguro (runtime remoto mock/dry_run).
 *
 * Usage: node tests/run-phase-val-0-admissions-pilot-safe.mjs
 * Output: tests/.phase-val-0-admissions-pilot-results.json
 */
...
const FIXTURE = path.join(ROOT, "tests/payloads/phase-val-0-admissions-pilot-safe.json");
const OUT_JSON = path.join(ROOT, "tests/.phase-val-0-admissions-pilot-results.json");

const fixture = JSON.parse(fs.readFileSync(FIXTURE, "utf8"));
const ENDPOINT = process.env.PHASE_VAL0_ENDPOINT || fixture.endpoint;
const RUN_NUM = String(process.env.PHASE_VAL0_RUN_ID || Date.now()).replace(/\D/g, "").slice(-8).padStart(8, "0");
...
function buildPayload({ phone, messageText, messageId }) {
  ...
  payload.message_id = messageId || `val0-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  return payload;
}
...
async function preflight() {
  ...
  const { status, body } = await post(buildPayload({ phone, messageText: "1", messageId: `val0-preflight-${RUN_NUM}` }));
  ...
  for (let i = 0; i < conv.turns.length; i++) {
    ...
    const { status, body } = await post(
      buildPayload({
        ...
        messageId: `val0-c${conv.id}-t${i + 1}-${RUN_NUM}`,
      }),
    );
...
async function main() {
  console.log(`VAL-0 admissions pilot (safe) → ${ENDPOINT}`);
  ...
  console.log(`\nVAL-0: ${passCount}/${fixture.conversations.length} conversations PASS`);
